...
res.edger <- et$table %>% as.data.frame() %>% dplyr::mutate(Symbol = cnts_fsym$Symbol,
                          FDR = p.adjust(et$table$PValue, method = "BH") ) 
res.edger$Gene_ID <- rownames(et$table)
...
## 
"SEP15 vs. 15-Sep the Gene Symbol" 
sum(genesbeta.de$Log2FoldChange == 0)
## 423 genes table /406 genestable 
...
```{r asso}
############## check the filtering of new gene lists #################
sum(isgs$Gene_ID %in% rownames(cnts.edger) )
nisgs <- sum(isgs$Gene_ID %in% rownames(cnts.edger) )
sum(genesbeta$Gene_ID %in% rownames(cnts.edger) )
ngenesbeta <- sum(genesbeta$Gene_ID %in% rownames(cnts.edger) )
...
## by pid
## basic clinical data
basic_cli_CH <- read.delim("~/Documents/gitlab/Cario_RNASeq_Microbiom_Inte/DataRaw/Basic_clinical_control_infected_untreated")
...
## whole isgs 
## now isgs has 246 genes 
gene_FunReg(as.matrix(isgs.rld.lin[,2:(nisgs + 1)]), isgs.rld.lin[,(nisgs + 5)], "Core ISGs", "Plasma Viral Load")

## betas
gene_FunReg(as.matrix(genesbeta.rld.lin[,2:(ngenesbeta + 1)]), genesbeta.rld.lin[,(ngenesbeta + 5)], "IFN-beta Genes", "Plasma Viral Load")


isgs.rld.lin[,(nisgs + 5)] == genesbeta.rld.lin[,(ngenesbeta + 5)]
################# CD4 Counts #################
## whole isgs 
gene_FunReg(as.matrix(isgs.rld.lin[,2:(nisgs + 1)]), isgs.rld.lin[,(nisgs + 4)], "Core ISGs", "CD4 Counts")

## betas
gene_FunReg(as.matrix(genesbeta.rld.lin[,2:(ngenesbeta + 1)]), genesbeta.rld.lin[,(ngenesbeta + 4)], "IFN-beta Genes", "CD4 Counts")
...
################# CD4 Counts #################
## whole isgs 
isgs.cd4 <- gene_FunRegRaw(as.matrix(isgs.rld.lin[,2:(nisgs + 1)]), isgs.rld.lin[,(nisgs + 4)], "Core ISGs", "CD4 Counts")

## betas
genesbeta.cd4 <- gene_FunRegRaw(as.matrix(genesbeta.rld.lin[,2:(ngenesbeta + 1)]), genesbeta.rld.lin[,(ngenesbeta + 4)], "IFN-beta Genes", "CD4 Counts")
...
############################ core ISGs#############################
isgs.edger.05 <- base::merge(res.edger.05, isgs, by = "Gene_ID") %>% dplyr::arrange(PValue)
dim(isgs.edger.05)
...
## DE genes in the core genesbeta list 
## directions of regulation 
genesbeta.edger.05 <- base::merge(res.edger.05, genesbeta, by = "Gene_ID") %>% dplyr::arrange(PValue)
...
############ viral load ################
## whole isgs 
gene_FunReg(as.matrix(rlog.isgs.05.raw), isgs.rld.lin[,(nisgs + 5)], "Core ISGs", "Plasma Viral Load")
gene_FunReg(as.matrix(rlog.isgs.05.1.raw), isgs.rld.lin[,(nisgs + 5)], "Core ISGs", "Plasma Viral Load")


## betas
gene_FunReg(as.matrix(rlog.genesbeta.05.raw), isgs.rld.lin[,(nisgs + 5)], "IFN-beta Genes", "Plasma Viral Load")
gene_FunReg(as.matrix(rlog.genesbeta.05.1.raw), isgs.rld.lin[,(nisgs + 5)], "IFN-beta Genes", "Plasma Viral Load")


isgs.rld.lin[,(nisgs + 5)] == genesbeta.rld.lin[,(ngenesbeta + 5)]
################# CD4 Counts #################
## whole isgs 
gene_FunReg(as.matrix(rlog.isgs.05.raw), isgs.rld.lin[,(nisgs + 4)], "Core ISGs", "CD4 Counts")
gene_FunReg(as.matrix(rlog.isgs.05.1.raw), isgs.rld.lin[,(nisgs + 4)], "Core ISGs", "CD4 Counts")

## betas
gene_FunReg(as.matrix(rlog.genesbeta.05.raw), isgs.rld.lin[,(nisgs + 4)], "IFN-beta Genes", "CD4 Counts")
gene_FunReg(as.matrix(rlog.genesbeta.05.1.raw), isgs.rld.lin[,(nisgs + 4)], "IFN-beta Genes", "CD4 Counts")
...
ggplot(mapping = aes(x = isgs.rld.lin[,(nisgs + 5)], 
  ...
  theme(
    ...
    annotate("text", x = mean(isgs.rld.lin[,(nisgs + 5)]), 
...
################### cd4 counts #####################
############# core ISGs ############# 
p.irs1 <-  ggplot(mapping = aes(x = isgs.rld.lin[,(nisgs + 4)], 
                                y = rlog.isgs.05.raw[,which(colnames(rlog.isgs.05.raw) %in% "IRS1") ])) + 
  ylab("rlog transformed counts") +
  ...
  theme(
    ...
    axis.text.x = element_text(face="bold", color="black", size=10, angle=0),
    axis.text.y = element_text(face="bold", color="black", size=10, angle=0) ) +
    annotate("text", x = mean(isgs.rld.lin[,(nisgs + 4)]), 
             y = quantile(rlog.isgs.05.raw[,which(colnames(rlog.isgs.05.raw) %in% "IRS1") ], 0.95), 
             label = "IRS1")
p.irs1 

scatter_cd4counts_isgs <- function(Symbol){
ggplot(mapping = aes(x = isgs.rld.lin[,(nisgs + 4)], 
                     y = rlog.isgs.05.raw[,which(colnames(rlog.isgs.05.raw) %in% Symbol) ])) + 
  ylab("rlog transformed counts") +
  xlab("Blood CD4 counts") +
  geom_point(colour = "gray15", size = 2, shape = 20) +
  geom_smooth(method = "loess", se = FALSE, colour = "steelblue") +
...
write.xlsx(isgs.save,
  ...
  ########  genes beta ##############
genesbeta[genesbeta$Symbol == "SEP15", ]
which(genesbeta$Symbol == "SEP15")
"15-Sep" %in% res.edger$Symbol
genesbeta[genesbeta$Symbol == "SEP15", 2] <- "15-Sep"
genesbeta.edger <- res.edger[ res.edger$Symbol %in% genesbeta$Symbol, ]
nrow(genesbeta.edger)
...
################### cnts edger of gene lists fdr 005 ###############
cnts.isgs.edger.05 <- base::merge(cnts.edger, isgs.edger.05[, 1:2], by = "Gene_ID") %>% 
                 select(-c(Symbol, logFC)) %>% column_to_rownames("Gene_ID")
...
cnts.genesbeta.edger.05 <- base::merge(cnts.edger, genesbeta.edger.05[, 1:2], by = "Gene_ID") %>% 
                 select(-c(Symbol, logFC)) %>% column_to_rownames("Gene_ID")
...
sum(isgs.rld.lin[,(nisgs + 4)] != clinical[,6])
...
clinical_order <- clinical_order[,c(1:18,24:25)]
